...
public class Vector<E> extends AbstractList<E> implements Cloneable
{
    ...
    public void ensureCapacity(int minCapacity)
    {
        if(elementData.length < minCapacity){
        	int newLength = Math.max(elementData.length, 1);
        	if(capacityIncrement == 0){
	        	final double growthFactor = 1.5;
	        	double newLengthDouble = (double) newLength;
	        	
	        	while(newLengthDouble < minCapacity){
	        		newLengthDouble *= growthFactor;
	        	}
	        	
	        	newLength = (int) Math.ceil(newLengthDouble);
        	} else {
    ...
    public void ensureCapacityOrig(int minCapacity)
    {
        if (elementData.length < minCapacity) {
